...
class CourseRepository implements IRepository{
    ...
    public function getSimilarCourses($id,$user_id){
        ...
        try{
            ...
            $pe_tag = Tag::where('course_id',$id)->where('course_type','App\Models\PrepareExams\Course')->where('deleted_at',null)->get();
            $tags = array();
            foreach ($pe_tag as $item){
                array_push($tags,$item->tag);
            }
            $courses_tag = Tag::where('deleted_at',null)->where('course_type','App\Models\PrepareExams\Course')->whereIn('tag',$tags)->get();
            $temp_courses = array();
            $courses = null;
